 ...
 * Credits: http://www.developer.com/java/other/article.php/3393581
...
public class GISCoordinate {
     ...
     */
    public GISCoordinate(double lat, double lon, boolean rad) throws Exception {
        if (rad) {
    ...
    public static GISCoordinate FromStringOrig(String s) throws Exception {
    ...
    public static GISCoordinate FromString(String latIn, String lonIn) throws Exception {
        ...
        if (latIn.indexOf(':')>0 || latIn.indexOf(':')>0){
            ...
            String latDir = "N";
            if (latIn.indexOf('(')>0){
                lat = latIn.substring(0, latIn.indexOf('('));
                latDir = Character.toString(latIn.charAt(latIn.indexOf('(')+1));
            }

            String lon = lonIn;
            String lonDir = "W";
            if (lonIn.indexOf('(')>0){
                lon = lonIn.substring(0, lonIn.indexOf('('));
                lonDir = Character.toString(lonIn.charAt(lonIn.indexOf('(')+1));
            }
        ...
        return new GISCoordinate(0,0,false);
    }
    ...
    private double parselatlon(String str) throws Exception {
        ...
        if (tok.hasMoreTokens()) {
            ...
            String secToken = "";
            try {
                degToken = tok.nextToken();
                ret+=Integer.parseInt(degToken); //degrees
            } catch (NumberFormatException e) {
                throw new Exception("Degrees must be an integer (" + degToken + ")");
            }
            if (tok.hasMoreTokens()) {
                int min=0;
                try {
                    minToken = tok.nextToken();
                    min=Integer.parseInt(minToken);//minutes
                } catch (NumberFormatException e) {
                    throw new Exception("Minutes must be an integer (" + minToken + ")");
                }
                if (min>=60) {
                    throw new Exception("Minutes must be less than 60 (" + minToken + ")");
                }
                ret+=min/60.0;
                if (tok.hasMoreTokens()) {
                    double sec=0;
                    try {
                        secToken = tok.nextToken();
                        sec=Double.parseDouble(secToken);//seconds
                    } catch (NumberFormatException e) {
                        throw new Exception("Seconds must be in decimal or integer format (" + secToken + ")");
                    }
                    if (sec>=60) {
                        throw new Exception("Seconds must be less than 60 (" + secToken + ")");
                    }
                    ret+=sec/3600.0;
                }
            }
    ...
    public static void main(String args[]) {
        try {
            ...
            g.printDEGMIN(System.out);
            System.out.println("\ntesting the toString function:"+GISCoordinate.FromStringOrig(g.toString()).toString());
        } catch (Throwable t) {
            t.printStackTrace();
        }
    }
