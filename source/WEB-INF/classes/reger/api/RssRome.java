        ...
        if (rstEvent!=null && rstEvent.length>0){
        	for(int i=0; i<rstEvent.length; i++){
                ...
                if (!isAutoNamedJobbie(entry, userSession.getAccount(), accountuserid)){
                    ...
                    //Guid
                    String entryurlGuid=reger.Entry.entryFileNameStatic(entry.logid, entry.eventid, entry.title);
                    ...
                    if (!entry.ishtml){
                        itemDescription = entry.comments.replaceAll( reger.Vars.CARRIAGERETURN + reger.Vars.LINEBREAK, "<br>");
                    }
