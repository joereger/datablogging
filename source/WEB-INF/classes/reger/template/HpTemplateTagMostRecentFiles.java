...
//        mb.append("<br>");
        ...
        //Get the list
        String sql="SELECT image.imageid, image.image, event.eventid, megalog.logid, event.title FROM image, megalog, event WHERE image.eventid=event.eventid AND event.logid=megalog.logid  AND event.accountid='"+ userSession.getAccount().getAccountid() +"' AND "+userSession.getAccountuser().LogsUserCanViewQueryend(userSession.getAccount().getAccountid())+" AND "+reger.Entry.sqlOfLiveEntry+" "+logidSql+" ORDER BY event.date DESC, image.order ASC LIMIT 0,"+maxinlist;
