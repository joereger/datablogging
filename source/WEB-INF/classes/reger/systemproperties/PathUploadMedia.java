...
public class PathUploadMedia extends SystemProperty{
    ...
    private String chooseDefaultPathByExaminingSystemDefault(){
        ...
        chosenFile = null;
        reger.core.Debug.debug(3, "PathUploadMedia.java", "Default value chosen by going to system root:" + defPath);
        return defPath;
    }

    public void save(){
        reger.core.Debug.debug(3, "PathUploadMedia.java", "save() overload called.");

        String tmpOut = propertyValue;
        reger.core.Debug.debug(3, "PathUploadMedia.java", "tmpOut=" + tmpOut);


        //Clean up the path
        tmpOut = reger.core.Util.cleanDirectorySlashesUseSystemFileSeparator(tmpOut);
        reger.core.Debug.debug(3, "PathUploadMedia.java", "tmpOut after cleanup=" + tmpOut);

        //Make sure there's a slash on it
        reger.core.Debug.debug(3, "PathUploadMedia.java", "tmpOut.substring(tmpOut.length()-1, tmpOut.length())=" + tmpOut.substring(tmpOut.length()-1, tmpOut.length()));
        if (!tmpOut.substring(tmpOut.length()-1, tmpOut.length()).equals(File.separator)){
            tmpOut=tmpOut + File.separator;
            reger.core.Debug.debug(3, "PathUploadMedia.java", "tmpOut after putting a slash at end=" + tmpOut);
        } else {
            reger.core.Debug.debug(3, "PathUploadMedia.java", "tmpOut did not need a slash added:" + tmpOut);
        }
    ...
    private void checkPath(){
        try{
            ...
            if (testDir !=null && testDir.exists() && testDir.canRead() && testDir.isDirectory() && testDir.canWrite()){
                ...
                reger.core.Debug.debug(3, "PathUploadMedia.java", "Directory exists:" + getPropertyValue());
            } else {
                //Try to create it
                reger.core.Debug.debug(3, "PathUploadMedia.java", "Directory does not exist:" + getPropertyValue() + "<br>Will try to create.");
                if (!createDir(getPropertyValue())){
                    //Choose a default value from the system root
                    reger.core.Debug.debug(3, "PathUploadMedia.java", "Unable to create:" + getPropertyValue());
                } else {
                    //Successfully created dir
                    reger.core.Debug.debug(3, "PathUploadMedia.java", "Successfully created:" + getPropertyValue());
                }
            }
        ...
        reger.core.Debug.debug(3, "PathUploadMedia.java", "PathUploadMedia.getPropertyValue()=" + getPropertyValue());
    }
    ...
    private void checkThumbnailsPath(){
        try{
            ...
            if (testDir !=null && testDir.exists() && testDir.canRead() && testDir.isDirectory() && testDir.canWrite()){
                //Do nothing, the directory is good
                reger.core.Debug.debug(3, "PathUploadMedia.java", "Thumbnails directory is OK.");
            } else {
                //Try to create it
                if (!createDir(getPropertyValue()+File.separator+"thumbnails")){
                    //Nothing to do... couldn't create
                    reger.core.Debug.debug(3, "PathUploadMedia.java", "Could not create thumbnails directory:" + getPropertyValue()+File.separator+"thumbnails");
                } else {
                    reger.core.Debug.debug(3, "PathUploadMedia.java", "Created thumbnails directory:" + getPropertyValue()+File.separator+"thumbnails");
                }
            }
        } catch (Exception e){
            e.printStackTrace();
    ...
    private boolean createDir(String path){
        try{
            reger.core.Debug.debug(3, "PathUploadMedia.java", "createDir("+path+") called.");
