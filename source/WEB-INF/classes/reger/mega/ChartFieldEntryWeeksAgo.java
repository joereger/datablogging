 ...
 * A dropdown field
 */
public class ChartFieldEntryWeeksAgo extends Field implements ChartField{
    ...
    public TreeMap getChartDataForField(int[] eventids) {
       ...
       if (eventids.length>0){
            ...
            Calendar endOfWeekInUserTimezone = reger.core.TimeUtils.xWeeksAgoEnd(nowinusertimezone, 0);
            ...
            if (rstEvents!=null && rstEvents.length>0){
                for(int i=0; i<rstEvents.length; i++){
                    ...
                    //Get the weeks ago
                    int daysago = reger.core.DateDiff.dateDiff("week", endOfWeekInUserTimezone, cal);
