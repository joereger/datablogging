 ...
 *
 */
public class MegaHtmlFormTopChiba {

    public static StringBuffer getHtml(UserSession userSession, reger.pageFramework.PageProps pageProps, boolean displayasadmin, javax.servlet.http.HttpServletRequest request, String timezoneid){
...
//        }
        ...
        try{
            ...
            mb.append(Vars.LINEBREAKCHARFORHTML);
            mb.append(EntryXmlRenderer.getWrappedInHtmlScript(pageProps.entry));
            mb.append(Vars.LINEBREAKCHARFORHTML);
            mb.append(Vars.LINEBREAKCHARFORHTML);
        } catch (Exception e){
        ...
        mb.append("<script type='text/javascript' src='"+pageProps.pathToAppRoot+"/js/leaveentrypage.js'></script>" + Vars.LINEBREAKCHARFORHTML);
        ...
        if (displayasadmin) {
            ...
            if (pageProps.action.equals("edit") || pageProps.action.equals("editsubmit")) {
                ...
                if (request.getParameter("mode")!=null && request.getParameter("mode").equals("editlayout")){
                    ...
                    mb.append(InfoBox.get(InfoBox.BOXTYPEINFO, pageProps.pathToAppRoot, "You are now customizing the activity-specific fields of this log.  You can change the arrangement of fields, edit the properties of a field, add new fields and remove fields.  When you're done, click Off to go back to normal entry mode."));
                }  else {
                    mb.append("<span style=\"background: #f6f6f6; float: right; border: 1px dotted #999; padding:3px; width: 25%;\">");
...
//        }
        ...
        if (displayasadmin) {
            ...
            //Start form
            if (!editLayout){
                mb.append("<form name='entryform' id='entryform' method='post' action='"+request.getRequestURI()+"' name='post'>");
                ...
                mb.append(MegaHtmlFormJscalendar.scriptHtmlSetup(pageProps.pathToAppRoot, initialDate));
        ...
        if (pageProps.action.equals("default") || pageProps.action.equals("defaultsubmit")) {
           ...
           if (!displayasadmin) {
            ...
            Debug.debug(4, "MegaHtmlFormTop.java", "MegaHtmlFormTop mm=" + mm + "<br>dateGmt="+ TimeUtils.dateformatfordb(pageProps.entry.dateGmt) + "<br>TimeUtils.gmttousertime(pageProps.entry.dateGmt, timezoneid)="+TimeUtils.gmttousertime(pageProps.entry.dateGmt, timezoneid));
            ...
            if (displayasadmin){
                if (!editLayout){
                    ...
                    mb.append(MegaHtmlFormJscalendar.theScript(pageProps));
                }
            } else {
                mb.append(TimeUtils.agoText(pageProps.entry.dateGmt));
            }
            mb.append("</td></tr>");
